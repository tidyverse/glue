...
test_that("inputs are concatenated, interpolated variables recycled", {
  expect_identical(c("testastring1", "testastring2"), glue("test", "a", "string", "{1:2}"))
})
...
test_that("glue works with single expressions", {
  foo <- "foo"
  expect_identical(foo, glue("{foo}"))

  foo <- 1L
  expect_identical(as.character(foo), glue("{foo}"))
...
test_that("glue works with repeated expressions", {
  foo <- "foo"
  expect_identical(paste(foo, foo), glue("{foo} {foo}"))

  foo <- 1L
  expect_identical(paste(as.character(foo), as.character(foo)), glue("{foo} {foo}"))
...
test_that("glue works with multiple expressions", {
  foo <- "foo"
  bar <- "bar"
  expect_identical(paste(foo, bar), glue("{foo} {bar}"))

  foo <- 1L
  bar <- 2L
  expect_identical(paste(as.character(foo), as.character(bar)), glue("{foo} {bar}"))
...
test_that("glue works with large outputs", {
  ...
  expect_identical(paste0(bar, additional), glue("{bar}", additional))
})
