...
[`glue()`](https://glue.tidyverse.org/reference/glue.md) repeatedly
...
expressions in [`glue()`](https://glue.tidyverse.org/reference/glue.md).
...
[`glue()`](https://glue.tidyverse.org/reference/glue.md) is called from.

Everything is simple when evaluating
[`glue()`](https://glue.tidyverse.org/reference/glue.md) in the global
environment:
...
Now we wrap [`glue()`](https://glue.tidyverse.org/reference/glue.md) in
our own simple function, `my_glue1()`. Notice that `my_glue1()` does not
...
eventual call to
[`glue()`](https://glue.tidyverse.org/reference/glue.md) have no access
to the execution environment of `my_glue2()`, which is the caller
environment of `my_glue1()`.

If you want your glue wrapper to behave like
[`glue()`](https://glue.tidyverse.org/reference/glue.md) itself and to
work as expected inside other functions, make sure it captures its
