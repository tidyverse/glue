...
- If the last argument of
  [`glue()`](https://glue.tidyverse.org/reference/glue.md) is empty, it
  is dropped ([\#320](https://github.com/tidyverse/glue/issues/320)).
  This makes it easy to structure
  [`glue()`](https://glue.tidyverse.org/reference/glue.md) calls with
  one argument per line, and to anticipate adding arguments:
...
- The `.envir` argument to
  ...
  [`glue_data()`](https://glue.tidyverse.org/reference/glue.md) really
  must be an environment now, as documented. Previously a list-ish
  object worked in some cases (by accident, not really by design). When
  you need to lookup values in a list-ish object, use `glue_data(.x =)`
  ([\#308](https://github.com/tidyverse/glue/issues/308),
  [\#317](https://github.com/tidyverse/glue/issues/317)). Ditto for
  [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) and
  [`glue_data_sql()`](https://glue.tidyverse.org/reference/glue_sql.md).
...
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  collapses an empty vector to `""` not `"NULL"`
...
- [`glue()`](https://glue.tidyverse.org/reference/glue.md),
  [`glue_data()`](https://glue.tidyverse.org/reference/glue.md),
  [`glue_col()`](https://glue.tidyverse.org/reference/glue_col.md), and
  [`glue_data_col()`](https://glue.tidyverse.org/reference/glue_col.md)
...
- [`glue_col()`](https://glue.tidyverse.org/reference/glue_col.md) gives
  special treatment to styling functions from the crayon package,
...
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md)
...
- [`glue()`](https://glue.tidyverse.org/reference/glue.md) gains a new
  `.comment` argument, to control the comment character
  ([\#193](https://github.com/tidyverse/glue/issues/193)).

- [`glue()`](https://glue.tidyverse.org/reference/glue.md) gains a new
  `.null` argument, to control the value to replace `NULL` values with
  ([\#217](https://github.com/tidyverse/glue/issues/217),
...
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  replaces missing values correctly when collapsing values
  ([\#185](https://github.com/tidyverse/glue/issues/185)).
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  always preserves the type of the column even in the presence of
...
- New [`glue_safe()`](https://glue.tidyverse.org/reference/glue_safe.md)
  and
  [`glue_data_safe()`](https://glue.tidyverse.org/reference/glue_safe.md)
  functions, safer versions of
  [`glue()`](https://glue.tidyverse.org/reference/glue.md) that do not
  execute code, only look up values (using
...
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  quotes lists of values appropriate to their type, rather than coercing
  all values to characters
  ([\#153](https://github.com/tidyverse/glue/issues/153))

- [`glue_data()`](https://glue.tidyverse.org/reference/glue.md) now
  implicitly coerces `.x` to a list.

- [`glue()`](https://glue.tidyverse.org/reference/glue.md) gains the
  `.trim` argument, like
  [`glue_data()`](https://glue.tidyverse.org/reference/glue.md).

- [`single_quote()`](https://glue.tidyverse.org/reference/quoting.md)
  [`double_quote()`](https://glue.tidyverse.org/reference/quoting.md)
  and [`backtick()`](https://glue.tidyverse.org/reference/quoting.md)
  all return `NA` for `NA` inputs
  ([\#135](https://github.com/tidyverse/glue/issues/135)).

- Improve [`trim()`](https://glue.tidyverse.org/reference/trim.md)’s
...
- [`glue()`](https://glue.tidyverse.org/reference/glue.md) now has a `+`
  method to combine strings.
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  collapses zero-length vector into `DBI::SQL("NULL")`
...
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  supports unquoting lists of Id objects.
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  quotes characters with NAs appropriately
  ([\#115](https://github.com/tidyverse/glue/issues/115)).
- [`glue_sql()`](https://glue.tidyverse.org/reference/glue_sql.md) now
  quotes Dates appropriately
...
- [`collapse()`](https://dplyr.tidyverse.org/reference/compute.html) has
  been renamed to
  [`glue_collapse()`](https://glue.tidyverse.org/reference/glue_collapse.md)
...
- Fix [`glue()`](https://glue.tidyverse.org/reference/glue.md) and
...
- [`glue()`](https://glue.tidyverse.org/reference/glue.md) now has a
  ...
  [`vignette("transformers")`](https://glue.tidyverse.org/articles/transformers.md)
  for more details and example transformer functions.

- [`glue()`](https://glue.tidyverse.org/reference/glue.md) now returns
  `NA` if any of the results are `NA` and `.na` is `NULL`. Otherwise
  `NA` values are replaced by the value of `.na`.

- [`trim()`](https://glue.tidyverse.org/reference/trim.md) to use the
  trimming logic from glue is now exported.
...
- [`glue()`](https://glue.tidyverse.org/reference/glue.md) is now easier
  to use when used within helper functions such as `lapply`.

- Fix when last expression in
  [`glue()`](https://glue.tidyverse.org/reference/glue.md) is NULL.
...
- [`glue()`](https://glue.tidyverse.org/reference/glue.md) always
...
- [`glue()`](https://glue.tidyverse.org/reference/glue.md) and
  [`glue_data()`](https://glue.tidyverse.org/reference/glue.md) can now
  take alternative delimiters to `{` and `}`. This is useful if you are
  writing to a format that uses a lot of braces, such as LaTeX.
...
- Fix [`glue()`](https://glue.tidyverse.org/reference/glue.md) to admit
  `.` as an embedded expression in a string
