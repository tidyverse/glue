...
You're not limited to using a bare symbol inside `{}`; it can basically be any little bit of R code:
...
There is considerably less syntactical noise and mystery, compared to `paste()` and `sprintf()`, for example.
...
* The "Get started" article (`vignette("glue", package = "glue")`) demonstrates more interesting features of `glue()` and `glue_data()`.
