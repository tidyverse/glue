...
To produce such text programmatically, the first step might be to generate the `\item{}{}` lines from a named character vector of column names and descriptions.
...
It can be hard to understand why `x` can't be found, when it is clearly available inside `named_chr_to_items_WRONG()`.
