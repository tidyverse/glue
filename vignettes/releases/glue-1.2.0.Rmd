...
Compared to equivalents like `paste()` and `sprintf()` it is easier to write
and less time consuming to maintain. It also has no non-base dependencies so is
...
`glue_data()` works like `glue()`, but instead of looking up it's variables
...
behavior for glue functions. Such as a `collapse_transformer()` which
...
* There are many other bug fixes and other minor improvements. You can see
  a complete list in the [release
  notes](https://github.com/tidyverse/glue/releases/tag/v1.2.0).

A big thanks goes to all the community members who contributed, opened issues
and used glue.
