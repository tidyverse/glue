...
This is easier to appreciate when you have `glue()` inside a function body (this example also has an alternative placement of the closing quote):
...
If you want a literal curly brace in your string, double it:
...
If you write your own custom wrapper around `glue()` and get unexpected "object not found" errors, read `vignette("wrappers", package = "glue")` to learn the right pattern for handling the evaluation environment.
